...
from .helper import strip_token, get_words_slice, get_ascii_ratio

class NetiNetiTrainer(object):
    ...
    def __init__(self, positive_training_file="data/names_in_context.txt",
        ...
        """
        self._positive_training_file = os.path.dirname(
            os.path.realpath(__file__)) + "/" + positive_training_file
        self._negative_training_file = os.path.dirname(
            os.path.realpath(__file__)) + "/" + negative_training_file
        self._sci_names_file = os.path.dirname(
            os.path.realpath(__file__)) + "/" + sci_names_file
    ...
    def _get_negative_featuresets(self):
        ...
        for first_word, second_word, third_word in neg_trigrams:
            ...
            if (first_word[0].isupper() and first_word[1:].islower()
                ...
                featuresets.append(
                    (self.taxon_features(bigram, ntokens, index, 1),
                     'not-a-taxon'))
                featuresets.append(
                    (self.taxon_features(first_word, ntokens, index, 0),
                     'not-a-taxon'))
                ...
                if third_word.islower():
                    trigram_count += 1
                    featuresets.append(
                        (self.taxon_features(trigram, ntokens, index, 2),
                         'not-a-taxon'))
            ...
            if second_word[0].isupper() and second_word[1:].islower():
                featuresets.append(
                    (self.taxon_features(second_word, ntokens, index + 1, 0),
                     'not-a-taxon'))
            if third_word[0].isupper() and third_word[1:].islower():
                featuresets.append(
                    (self.taxon_features(third_word, ntokens, index + 2, 0),
                     'not-a-taxon'))
    ...
    def _get_positive_featuresets(self, positive_training_data):
        ...
        for data in positive_training_data:
            ...
            if len(name_tokens[0]) > 1 and name_tokens[0][1] != ".":
                ...
                features = self.taxon_features(abbr_name, context_tokens,
                                               index, span)
                featuresets.append((features, 'taxon'))
        return featuresets
    ...
    def taxon_features(self, token, context_array, index, span):
        ...
        features["first_word_in_wl"] = words[0].lower() in self._white_list
        features["second_word_in_wl"] = \
           get_words_slice(words, 1, 0, 1000).lower() in self._white_list
        features["third_word_in_wl"] = \
           get_words_slice(words, 2, 0, 1000).lower() in self._white_list
        for i in range(context_span):
            features[str(i + 1) + "_context"] = \
                strip_token(
                    get_words_slice(context_array, index + span + i + 1,
                                    0, 1000))
            if index + i - context_span < 0:
                ...
                features[str(i - context_span) + "_context"] = \
                    strip_token(get_words_slice(context_array,
                                                index + i - context_span,
                                                0, 1000))
        try:
            features["1up_2dot_rest_lower_case"] = (
                token[0].isupper() and
                token[1] is "." and
                token[2] is " " and
                token[3:].islower())
